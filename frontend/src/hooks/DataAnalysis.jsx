...
export default function DataAnalysis() {
    ...
    try {
      ...
      if (isDegree) {
        ...
        if (selectedKeys.length > 0)
          qs += `&analysis=${encodeURIComponent(selectedKeys.join(','))}`;
        if (filterExamMonth)
          qs += `&exam_month=${encodeURIComponent(filterExamMonth)}`;
        if (filterExamYear)
          qs += `&exam_year=${encodeURIComponent(filterExamYear)}`;
        if (filterConvocation)
          qs += `&convocation_no=${encodeURIComponent(filterConvocation)}`;
        if (filterInstitute)
          qs += `&institute=${encodeURIComponent(filterInstitute)}`;
      }

      const res = await authFetch(
        `${apiBase}/data-analysis/?${qs}&_=${Date.now()}`,
      );
  ...
  const loadRecordsForKey = async (key, groupType = null) => {
    try {
      ...
      if (isDegree) {
        ...
        const res = await authFetch(
          `${apiBase}/data-analysis/?service=Degree&group_type=${encodeURIComponent(
            groupType || '',
          )}&group_key=${encodeURIComponent(key)}`,
        );
      ...
      let endpoint = '';
      if (service === 'PROVISIONAL')
        endpoint = `${apiBase}/provisional/?search=${encodeURIComponent(key)}`;
      else if (isEnrollment)
        endpoint = `${apiBase}/enrollments/?search=${encodeURIComponent(key)}`;
      else
        endpoint = `${apiBase}/provisional/?search=${encodeURIComponent(key)}`;
  ...
  const deleteSelected = async (idsOverride = null) => {
    ...
    if (
      !confirm(
        `Delete ${idsArray.length} selected record(s)? This cannot be undone.`,
      )
    )
      return;
    try {
      const endpointRoot = isDegree
        ? 'degrees'
        : isEnrollment
        ? 'enrollments'
        : 'provisional';
      await Promise.all(
        idsArray.map(async (id) => {
          const res = await authFetch(`${apiBase}/${endpointRoot}/${id}/`, {
            method: 'DELETE',
          });
          if (!res.ok) {
  ...
  const deleteDuplicatesKeepOne = async (key) => {
    if (!confirm(`Delete duplicate records for '${key}', keeping one record?`))
      return;
    try {
      const endpointRoot = isDegree
        ? 'degrees'
      ...
      if (isDegree) {
        ...
        const res = await authFetch(
          `${apiBase}/${endpointRoot}/?search=${encodeURIComponent(key)}`,
        );
        const data = await res.json();
      ...
      for (let i = 1; i < rows.length; i++) {
        const id = rows[i].id;
        const dres = await authFetch(`${apiBase}/${endpointRoot}/${id}/`, {
          method: 'DELETE',
        });
    ...
    try {
      if (isDegree) {
        const newName = prompt(
          'Student name',
          row.student_name_dg || row.student_name || '',
        );
        ...
        const res = await authFetch(`${apiBase}/degrees/${row.id}/`, {
          method: 'PATCH',
    ...
    <div className="p-4 space-y-4">
      ...
      {isDegree && (
        <div className="p-4 bg-white rounded text-sm space-y-3 text-black border border-gray-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <div className="underline font-medium mb-1">
              ...
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.DUPLICATE_ENROLL_NAME_MONTH_YEAR}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        DUPLICATE_ENROLL_NAME_MONTH_YEAR: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>
                    Duplicate Enrollment + Name + Exam Month + Exam Year
                  </span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.ENROLLMENT_SAME_NAME_DIFFER}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        ENROLLMENT_SAME_NAME_DIFFER: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Same Enrollment but Different Name</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.ENROLLMENT_NAME_DIFF_YEARS}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        ENROLLMENT_NAME_DIFF_YEARS: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Enrollment+Name Same but Exam Year Different</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.ENROLLMENT_NAME_DIFF_MONTHS}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        ENROLLMENT_NAME_DIFF_MONTHS: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Enrollment+Name Same but Exam Month Different</span>
                </span>
              </label>
              <label className="block text-sm">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.NAME_SAME_DIFFERENT_ENROLLMENT}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        NAME_SAME_DIFFERENT_ENROLLMENT: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Same Name but Different Enrollment</span>
                </span>
              </label>
            </div>
              ...
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.STATS_CONVOCATION}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        STATS_CONVOCATION: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Number of Degrees by Convocation</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.STATS_COURSE}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        STATS_COURSE: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Course-wise Total Degrees</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.STATS_COLLEGE}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        STATS_COLLEGE: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>College-wise Total Degrees</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.STATS_YEAR}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        STATS_YEAR: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Year-wise Degree Count</span>
                </span>
              </label>
              <label className="block text-sm">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.STATS_MONTH}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        STATS_MONTH: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Month-wise Degree Count</span>
                </span>
              </label>
            </div>
              ...
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.MISSING_CONVOCATION}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        MISSING_CONVOCATION: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Degrees Missing Convocation</span>
                </span>
              </label>
              <label className="block text-sm mb-1">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.MISSING_EXAM_MONTH_OR_YEAR}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        MISSING_EXAM_MONTH_OR_YEAR: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Degrees Missing Exam Month/Year</span>
                </span>
              </label>
              <label className="block text-sm">
                <span className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2"
                    checked={analysisOptions.DUPLICATE_DG_SR_NO}
                    onChange={(e) =>
                      setAnalysisOptions({
                        ...analysisOptions,
                        DUPLICATE_DG_SR_NO: e.target.checked,
                      })
                    }
                  />{' '}
                  <span>Duplicate Degree Serial Number</span>
                </span>
              </label>
            </div>
          </div>
          <div className="pt-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  placeholder="Convocation no (e.g., 33)"
                  aria-label="Filter Convocation"
                  className="w-full border px-2 py-1 rounded bg-white text-black placeholder-gray-400"
              ...
              <div>
                ...
                <input
                  placeholder="e.g., Jan, Feb or 01"
                  aria-label="Exam Month"
                  className="w-full border px-2 py-1 rounded bg-white text-black placeholder-gray-400"
                  value={filterExamMonth}
                  onChange={(e) => setFilterExamMonth(e.target.value)}
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">
      ...
      {report && !hideSummaryTable && (
        <div className="space-y-3">
          <div>
            ...
            <ul className="list-disc ml-6 text-sm">
              {report.summary &&
                ...
                Object.entries(report.summary.by_type).map(([k, v]) => (
                  <li key={k}>
                    {k}: {v}
                  </li>
                ))}
          ...
          <div
            className="bg-white rounded p-3 overflow-auto border border-gray-200"
            style={{ maxHeight: 320 }}
          >
            <table className="min-w-full text-sm text-black">
              <thead>
                <tr>
                  <th className="text-left pr-4 pb-1 border-b border-gray-200">
                    Key
                  </th>
                  {isDegree && (
                    <th className="text-left pr-4 pb-1 border-b border-gray-200">
              ...
              <tbody>
                {(report.issues || report.duplicates || []).map((it, i) => (
                  <tr key={i} className="border-b border-gray-200">
                    ...
                    {isDegree && (
                      <td className="pr-4 py-1 text-sm text-gray-700">
                        {getConvocationText(it)}
                      </td>
                    )}
                    ...
                    <td className="pl-4 py-1">
                      {service === 'PROVISIONAL' &&
                        (it.type?.includes('DUPLICATE') ||
                          it.type === 'DUPLICATE_PRV_NUMBER') && (
                          <div className="flex gap-2">
                            <button
                              onClick={() => loadRecordsForKey(it.key)}
                              className="px-2 py-0.5 bg-green-600 text-white rounded text-xs"
                            >
                              View
                            </button>
                            <button
                              onClick={() => deleteDuplicatesKeepOne(it.key)}
      ...
      {isDegree &&
        report?.issues?.some(
          (issue) => Array.isArray(issue.records) && issue.records.length > 0,
        ) && (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 px-4 py-3 border-b border-gray-100">
                    ...
                    <div className="text-xs text-gray-600">
                      Convocation(s): {getConvocationText(issue)}
                    </div>
                  </div>
                            ...
                            <td className="p-2 border-b border-gray-100">
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="px-2 py-0.5 bg-yellow-600 text-white rounded"
                                >
                                  Edit
                                </button>
                                <button
                                  onClick={() => deleteSingleRecord(row.id)}
                  ...
                  <tr key={r.id} className="border-b border-gray-200">
                    ...
                    {service === 'DEGREE' ? (
                      ...
                      <>
                        ...
                        <td className="pr-4 py-1">
                          {r.doc_rec_id || r.doc_rec || ''}
                        </td>
                        <td className="pr-4 py-1">
                          {r.enrollment_no || r.enrollment || ''}
                        </td>
