...
export default function AuthLeave() {
  ...
  function normalizeAlloc(a) {
    return {
      id: a.id || a.pk || null,
      emp_id:
        (a.emp_id ?? (a.emp ? a.emp.emp_id || a.emp : null)) ||
        (a.emp === null ? null : a.emp),
      leave_code:
        a.leave_code ||
        a.leave_type ||
        (a.leave_type && a.leave_type.leave_code) ||
        null,
      leave_name:
        a.leave_type_name || (a.leave_type && a.leave_type.leave_name) || null,
      period_id:
        (a.period_id ?? (a.period && (a.period.id || a.period))) || null,
      period_name: a.period_name || (a.period && a.period.period_name) || null,
      allocated: a.allocated ?? a.allotted ?? 0,
      allocated_start_date:
        a.allocated_start_date || a.allocation_start_date || null,
  ...
  async function createAllocation(payload) {
    ...
    if (payload.allocated_start_date)
      body.allocated_start_date = payload.allocated_start_date;
    if (payload.allocated_end_date)
      body.allocated_end_date = payload.allocated_end_date;
  ...
  function openEditAllocation(a) {
    ...
    const selectedPeriodObj = periods.find(
      (p) => String(p.id) === String(a.period_id),
    );
    ...
    setAllocForm({
      ...
      allocated_start_date:
        a.allocated_start_date || selectedPeriodObj?.start_date || '',
      allocated_end_date:
        a.allocated_end_date || selectedPeriodObj?.end_date || '',
      apply_to: a.apply_to || (a.emp_id ? 'PARTICULAR' : 'ALL'),
    });
    setShowAllocForm(true);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  async function saveAllocation(e) {
    ...
    if (allocForm.apply_to === 'PARTICULAR' && !allocForm.emp_id) {
      ...
      if (
        !window.confirm(
          'You did not enter Emp ID. If you intend to create allocation for ALL employees choose Apply To = ALL. Proceed as ALL?',
        )
      ) {
        return;
      }
  ...
  async function confirmDeleteAllocation(a) {
    ...
    if (
      !window.confirm(
        `Delete allocation ${a.leave_code} for ${a.emp_id || 'All'} (period ${
          a.period_id
        })?`,
      )
    )
  ...
  async function submitPeriod(e) {
    ...
    try {
      ...
      alert(
        'Failed to save period: ' + (err.response?.data?.detail || err.message),
      );
    }
  }
      ...
      <div className="mb-4">
        <button
          className={`px-3 py-1 mr-2 ${
            tab === 'allocations' ? 'bg-blue-600 text-white' : 'bg-gray-200'
          }`}
        ...
        <button
          className={`px-3 py-1 ${
            tab === 'periods' ? 'bg-blue-600 text-white' : 'bg-gray-200'
          }`}
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                onClick={() => loadAllocations(selectedPeriod)}
              >
                Refresh
              </button>
            </div>
                ...
                {allocs.length === 0 ? (
                  ...
                  allocs.map((a) => (
                    <tr
                      key={
                        a.id ||
                        `${a.emp_id || 'all'}-${a.period_id}-${a.leave_code}`
                      }
                    ...
                    >
                      ...
                      <td className="py-2 px-3">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => confirmDeleteAllocation(a)}
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div>
                <button
                  onClick={() => {
                    setShowAllocForm((s) => !s);
                    if (showAllocForm) cancelEditAllocation();
                  }}
                  className="px-3 py-1 bg-blue-600 text-white rounded"
                >
                  {showAllocForm ? 'Close' : 'Add Allocation'}
                </button>
              </div>
            </div>
            ...
            {showAllocForm && (
              <form
                ...
                className="max-w-3xl p-3 border rounded"
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs">Apply To</label>
                    <select
                      value={allocForm.apply_to}
                      onChange={(e) =>
                        setAllocForm((f) => ({
                          ...f,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAllocForm((f) => ({ ...f, emp_id: e.target.value }))
                      }
                      disabled={allocForm.apply_to === 'ALL'}
                    />
                  </div>
                      ...
                      onChange={(e) => {
                        ...
                        setAllocForm((f) => ({
                          ...
                          allocated_start_date:
                            ...
                            f.allocated_start_date,
                          allocated_end_date:
                            selectedPeriodObj?.end_date || f.allocated_end_date,
                        }));
                      }}
                      className="p-2 border rounded w-full"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAllocForm((f) => ({
                          ...f,
                          allocated_start_date: e.target.value,
                        }))
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAllocForm((f) => ({
                          ...f,
                          allocated_end_date: e.target.value,
                        }))
                      }
                    />
                  </div>
                  ...
                  <div className="md:col-span-3 flex items-center space-x-2 mt-2">
                    ...
                    <div className="text-sm text-gray-600 ml-3">
                      Tip: choose <strong>All</strong> in Apply To to create a
                      global allocation for everyone.
                    </div>
                  </div>
                </div>
              </form>
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-xs text-gray-500">
              Showing: Code, Name, Main Type, Day Value, Session, Annual, Half
              Day | Records: {types.length}
            </span>
          </div>
          <div className="mb-4">
            <form
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, leave_code: e.target.value }))
                    }
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, leave_name: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, main_type: e.target.value }))
                    }
                    maxLength="10"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    max="9999"
                    value={typeForm.day_value}
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, day_value: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="Mode"
                    className="p-2 border rounded w-full"
                    value={typeForm.session}
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, session: e.target.value }))
                    }
                    maxLength="10"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTypeForm((f) => ({
                        ...f,
                        annual_allocation: e.target.value,
                      }))
                    }
                  />
                </div>
              ...
              <div className="flex items-center space-x-3 mb-3">
                <label className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={typeForm.is_half}
                    onChange={(e) =>
                      setTypeForm((f) => ({ ...f, is_half: e.target.checked }))
                    }
                    className="w-4 h-4"
                  />
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading && types.length === 0 ? (
                  ...
                  <tr>
                    <td colSpan="7" className="p-4 text-center text-gray-500">
                      No leave types found. Click "Create" to add a new leave
                      type.
                      <br />
                      ...
                      <tr
                        ...
                        onClick={() => {
                          ...
                          setTypeForm({
                            ...
                            session: t.session || '',
                            annual_allocation:
                              t.annual_allocation ?? t.annual_limit ?? '',
        ...
        <div>
          ...
          <div className="mb-4">
            <form
              ...
              className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-xl"
            >
              <input
                name="period_name"
                placeholder="Name"
                ...
                onChange={(e) =>
                  setPeriodForm((f) => ({ ...f, period_name: e.target.value }))
                }
              />
              <input
                name="start_date"
                ...
                onChange={(e) =>
                  setPeriodForm((f) => ({ ...f, start_date: e.target.value }))
                }
              />
              <input
                name="end_date"
                ...
                onChange={(e) =>
                  setPeriodForm((f) => ({ ...f, end_date: e.target.value }))
                }
              />
              <textarea
                name="description"
                ...
                onChange={(e) =>
                  setPeriodForm((f) => ({ ...f, description: e.target.value }))
                }
              />
              <div className="md:col-span-3 flex items-center space-x-2">
                <button className="bg-green-600 text-white px-3 py-1 rounded">
                  {editingPeriodId ? 'Save' : 'Create Period'}
                </button>
                {editingPeriodId && (
                  <button
                    type="button"
