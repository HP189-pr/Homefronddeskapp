...
const PermissionsAdmin = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [pD, rD, aD, mD, menuD] = await Promise.all([
        pR.json(),
        rR.json(),
        aR.json(),
        mR.json(),
  ...
  const remove = async (id) => {
    ...
    const res = await authFetch('/api/admin/rights/permissions/' + id, {
      method: 'DELETE',
    });
  ...
  const createAssignment = async () => {
    if (!assignForm.userid || !assignForm.roleid)
      return alert('userid and roleid required');
    const payload = {
      userid: parseInt(assignForm.userid, 10),
      roleid: parseInt(assignForm.roleid, 10),
    };
  ...
  const removeAssignment = async (id) => {
    ...
    const res = await authFetch('/api/admin/rights/assignments/' + id, {
      method: 'DELETE',
    });
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        <select
          value={form.moduleid}
          onChange={(e) =>
            setForm({ ...form, moduleid: e.target.value, menuid: '' })
          }
          className="p-2 border"
        >
        ...
        >
          ...
          {(() => {
            const filtered = menus.filter(
              (m) =>
                !form.moduleid || String(m.moduleid) === String(form.moduleid),
            );
            if (!filtered.length)
              return [
                <option key="__nomenu__" value="" disabled>
                  No menus found for selected module
                </option>,
              ];
        ...
        <input
          placeholder="action (optional e.g. view)"
          value={form.action}
      ...
      <div className="space-y-2 mb-4">
        {assignments.map((a) => (
          ...
          >
            <div>
              id:{a.id} user:{a.userid} role:{a.roleid}
            </div>
            <button
      ...
      <div className="space-y-2">
        {perms.map((p) => (
          ...
          >
            <div>
              role:{p.roleid} module:{p.moduleid ?? 'any'} menu:
              {p.menuid ?? 'any'} institute:{p.instituteid ?? 'any'} action:
              {p.action ?? 'any'}
            </div>
            <button
              onClick={() => remove(p.permissionid)}
              className="p-1 bg-red-600 text-white rounded"
            >
              Remove
            </button>
