...
const UsersAdmin = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      try {
        console.debug(
          'UsersAdmin.load: response status=',
          res.status,
          'ok=',
          res.ok,
        );
      ...
      if (!res.ok) {
        ...
        try {
          console.debug('UsersAdmin.load: payload=', payload);
        } catch (e) {}
  ...
  useEffect(() => {
    (async () => {
      ...
      const [rD, mD, menuD] = await Promise.all([
        rR.json(),
        mR.json(),
        menuR.json(),
      ]);
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        ...
        if (!payload.userid || !payload.usrpassword) {
          alert('Userid and password required');
          return;
        }
        const res = await authFetch('/api/admin/users', {
          method: 'POST',
  ...
  const openRights = async (u) => {
    ...
    if (res.ok) {
      ...
      setPermForm((p) => ({
        ...p,
        roleid: firstRole ? String(firstRole) : '',
      }));
      // Preload permissions for these roles
      const map = {};
      for (const a of d.assignments || []) {
        const r = await authFetch(
          `/api/admin/rights/permissions?roleid=${a.roleid}`,
        );
  ...
  const toggleRole = async (roleid) => {
    ...
    try {
      if (hasRole(roleid)) {
        ...
        if (!a) return;
        const res = await authFetch(`/api/admin/rights/assignments/${a.id}`, {
          method: 'DELETE',
        });
        ...
        const res = await authFetch('/api/admin/rights/assignments', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userid: activeUser.id, roleid }),
        });
        ...
        const r = await authFetch(
          `/api/admin/rights/permissions?roleid=${roleid}`,
        );
        const pd = r.ok ? await r.json() : { permissions: [] };
  ...
  const addPermission = async () => {
    try {
      ...
      const payload = {
        ...
        instituteid: permForm.instituteid
          ? parseInt(permForm.instituteid, 10)
          : null,
      };
  ...
  const removePermission = async (roleid, permissionid) => {
    try {
      ...
      const res = await authFetch(
        `/api/admin/rights/permissions/${permissionid}`,
        { method: 'DELETE' },
      );
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div>
          ...
          <button onClick={load} className="ml-2 px-3 py-2 bg-gray-200 rounded">
            Refresh
          </button>
        </div>
      </div>
            ...
            <div className="space-y-2">
              ...
              {!editing && (
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, usrpassword: e.target.value })
                  }
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        const res = await authFetch(
                          '/api/admin/users/' + editing.id + '/password',
                          {
                            method: 'PATCH',
                            headers: { 'Content-Type': 'application/json' },
          ...
          <div className="flex justify-between items-center mb-3">
            <div>
              <h3 className="text-lg font-semibold">
                Rights for {activeUser.first_name || activeUser.userid}
              </h3>
              <div className="text-sm text-gray-500">
                UserID: {activeUser.userid} · Email: {activeUser.email || '-'} ·
          ...
          <div className="space-y-4 overflow-auto h-[80%] pr-2">
            ...
            <section className="border rounded p-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setPermForm({ ...permForm, roleid: e.target.value })
                  }
                  className="p-2 border"
                >
                  <option value="">Select role</option>
                  {assignments.map((a) => (
                    <option key={a.id} value={a.roleid}>
                      {roles.find((r) => r.roleid === a.roleid)?.name ||
                        a.roleid}
                    </option>
                  ))}
                </select>
                <select
                  value={permForm.moduleid}
                  onChange={(e) =>
                    setPermForm({
                      ...permForm,
                ...
                <select
                  value={permForm.menuid}
                  onChange={(e) =>
                    setPermForm({ ...permForm, menuid: e.target.value })
                  }
                  className="p-2 border"
                >
                  ...
                  {menus
                    .filter(
                      (m) =>
                        !permForm.moduleid ||
                        String(m.moduleid) === String(permForm.moduleid),
                    )
                    .map((m) => (
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <input
                    value={permForm.instituteid}
                    onChange={(e) =>
                      setPermForm({ ...permForm, instituteid: e.target.value })
                    }
                    placeholder="instituteid (optional)"
            ...
            <section className="border rounded p-3">
              ...
              <p className="text-sm text-gray-500 mb-2">
                Permissions are granted via roles above. To add more, go to User
                Rights page.
              </p>
            ...
            <section className="border rounded p-3">
              ...
              {assignments.length === 0 ? (
                <div className="text-sm text-gray-600">
                  Assign a role to manage permissions.
                </div>
              ) : (
...
const EffectivePermissions = ({ assignments, modules, menus, authFetch }) => {
  ...
  const merged = useMemo(() => {
    const all = [];
    Object.values(permsByRole).forEach((arr) => {
      all.push(...arr);
    });
    // group by module/menu/action
    const key = (p) =>
      `${p.moduleid || 'any'}|${p.menuid || 'any'}|${p.action || 'any'}|${
        p.instituteid || 'any'
      }`;
    const map = new Map();
  ...
  const moduleName = (id) =>
    id ? modules.find((m) => m.moduleid === id)?.name || id : 'any';
  const menuName = (id) =>
    id ? menus.find((m) => m.menuid === id)?.name || id : 'any';
