...
const Inventory = () => {
  ...
  const loadTabData = async () => {
    ...
    try {
      if (activeTab === 'stock') {
        ...
        const [itemsData, inwardData] = await Promise.all([
          getItems(),
          getInward(),
        ]);
        setItems(
        ...
        setInwardEntries(
          Array.isArray(inwardData) ? inwardData : inwardData?.results || [],
        );
      } else if (activeTab === 'outward') {
        const [itemsData, outwardData] = await Promise.all([
          getItems(),
          getOutward(),
        ]);
        setItems(
        ...
        setOutwardEntries(
          Array.isArray(outwardData) ? outwardData : outwardData?.results || [],
        );
  ...
  const handleItemDelete = async (id) => {
    if (
      !window.confirm('Delete this item? This will also affect stock entries.')
    )
      return;
    setLoading(true);
    try {
  ...
  const handleOutwardSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        received_qty: outwardForm.received_qty
          ? parseInt(outwardForm.received_qty)
          : null,
    ...
    <div className="space-y-4">
      <div className="bg-white rounded shadow p-4">
        <h2 className="text-xl font-bold mb-4">
          {editingInward ? 'Edit' : 'Add'} Inward Entry
        </h2>
        <form
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setInwardForm({ ...inwardForm, item: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              required
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setInwardForm({ ...inwardForm, qty: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              min="1"
              required
  ...
  const renderOutwardEntry = () => (
    <div className="space-y-4">
      <div className="bg-white rounded shadow p-4">
        <h2 className="text-xl font-bold mb-4">
          {editingOutward ? 'Edit' : 'Add'} Outward Entry
        </h2>
        <form
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setOutwardForm({ ...outwardForm, item: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              required
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOutwardForm({ ...outwardForm, qty: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              min="1"
              required
          ...
          <div>
            <label className="block text-sm font-medium mb-1">
              Receiver Name / Institute <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={outwardForm.receiver}
              onChange={(e) =>
                setOutwardForm({ ...outwardForm, receiver: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              required
            />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOutwardForm({ ...outwardForm, received_qty: e.target.value })
              }
              className="w-full border px-3 py-2 rounded"
              min="0"
