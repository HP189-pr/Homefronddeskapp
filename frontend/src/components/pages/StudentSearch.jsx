...
export default function StudentSearch() {
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span
        className={`inline-block px-3 py-1 text-xs font-semibold rounded-full bg-${color}-100 text-${color}-800`}
      >
        {status.replace(/_/g, ' ')}
      </span>
    );
    ...
    degree: {
      ...
      columns: [
        ...
        { key: 'specialisation', label: 'Specialisation' },
        {
          key: 'passing_year',
          label: 'Passing Year',
          render: (val, row) => row.passing_year || renderExamPeriod(row),
        },
        { key: 'class_obtain', label: 'Class' },
        { key: 'convocation_no', label: 'Conv' },
        {
          key: 'convocation_period',
          label: 'Convocation Month-Year',
          render: (val, row) => row.convocation_period || '-',
        },
      ],
    },
  };
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="space-y-6">
                  <DetailStat
                    label="Student Name"
                    value={generalInfo.student_name}
                    large
                  />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <DetailStat
                      label="Enrollment No"
                      value={generalInfo.enrollment_no}
                    />
                    <DetailStat
                      label="Temp Enrollment"
                      value={generalInfo.temp_enrollment_no}
                    />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <DetailStat
                      ...
                      value={formatISODate(generalInfo.admission_date)}
                    />
                    <DetailStat
                      label="Enrollment Date"
                      value={formatISODate(generalInfo.enrollment_date)}
                    />
                    <DetailStat label="ABC ID" value={generalInfo.abc_id} />
                    <DetailStat
                      label="Aadhaar No"
                      value={generalInfo.aadhar_no}
                    />
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="space-y-6">
                  <DetailStat
                    ...
                    value={generalInfo.institute_name}
                    large
                  />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <DetailStat
                      label="Institute Code"
                      value={generalInfo.institute_code}
                    />
                    <DetailStat
                      label="Institute City"
                      value={generalInfo.institute_city}
                    />
                    <DetailStat
                      label="Main Course"
                      value={generalInfo.maincourse}
                    />
                    <DetailStat
                      label="Specialisation"
                      value={generalInfo.subcourse}
                    />
                  </div>
                  <DetailStat
                    label="Institute Address"
                    value={generalInfo.institute_address}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              {[
                ...
                'institutional_verification',
                'degree',
              ].map((key) => renderServiceSection(key))}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg border-l-4 border-green-500">
                  ...
                  <p className="text-3xl font-bold text-green-700">
                    ₹
                    {(studentData?.fees?.total_fees ?? 0).toLocaleString(
                      'en-IN',
        ...
        {searched && !loading && !studentData && !error && (
          <div className="bg-white rounded-lg shadow-lg p-12 text-center">
            ...
            <p className="text-xl text-gray-600">
              No student found with this enrollment number
            </p>
          </div>
        )}
      </div>
