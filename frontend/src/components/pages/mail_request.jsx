...
} from '../../services/mailRequestService.js';
...
const MailRequestPage = ({ onToggleSidebar, onToggleChatbox }) => {
  ...
  const applyUpdate = async (rowId, overrideDraft = null) => {
    ...
    if (!row) return;
    const draftSource =
      overrideDraft !== null ? overrideDraft : drafts[rowId] || {};
    const payload = {};
  ...
  const handleRefresh = async (rowId) => {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((item) => (item.id === rowId ? refreshed : item)),
      );
      setDrafts((prev) => {
        const copy = { ...prev };
        delete copy[rowId];
      ...
      setFlashMessage(
        'error',
        err.message || 'Failed to refresh verification.',
      );
    } finally {
  ...
  const handleBulkRefresh = async () => {
    ...
    try {
      ...
      setFlashMessage(
        'success',
        'Verification refreshed for selected submissions.',
      );
    } catch (err) {
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            {panelOpen && selectedAction === ACTIONS[1] && (
              <div className="p-4 space-y-3 text-sm text-gray-700">
                <p>
                  Use bulk tools to refresh multiple submissions at once. Select
                  the rows from the records table below and run a bulk refresh
                  to re-check enrollment and student name verification.
                </p>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">
                    Currently selected: {selectedIds.length} submission(s).
                  </span>
                </div>
                <ul className="list-disc pl-5 text-xs text-gray-500 space-y-1">
                  ...
                  <li>
                    Bulk refresh re-runs the verification logic for each
                    selected submission.
                  </li>
                  <li>
                    Status or remark edits still require saving row by row.
                  </li>
                </ul>
              </div>
            )}
                ...
                {activeRow ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="md:col-span-2 flex justify-end">
                      <button
                        onClick={() =>
                          activeRow && applyUpdate(activeRow.id, editForm)
                        }
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead className="bg-white">
                  <tr>
                    <th className="px-3 py-1 text-left text-red-600 font-semibold text-sm">
                      {' '}
                    </th>
                    <th className="px-3 py-1 text-left text-red-600 font-semibold text-sm">
                      mail_req_no
                ...
                <tbody>
                  ...
                  {!loading &&
                    ...
                    sortedRows.map((row) => {
                      ...
                      const mailStatus =
                        draft.mail_status ?? row.mail_status ?? '';
                      ...
                      return (
                        <tr
                          key={row.id}
                          className={`border-t border-gray-100 align-top cursor-pointer ${
                            activeRow?.id === row.id
                              ? 'bg-indigo-50'
                              : 'hover:bg-gray-50'
                          }`}
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-gray-800">
                            {row.rec_institute_name || 'N/A'}
                          </td>
                          <td className="px-3 py-2 break-all text-gray-700">
                            {row.rec_official_mail || 'N/A'}
                          </td>
                          <td className="px-3 py-2 text-gray-800">
                            {row.rec_ref_id || 'N/A'}
                          </td>
                          <td className="px-3 py-2 text-gray-800">
                            {row.send_doc_type || 'N/A'}
                          </td>
                          <td className="px-3 py-2">
                            <select
