...
} from '../../services/transcriptreqService.js';
// ...existing code...
...
const TranscriptRequestPage = ({ onToggleSidebar, onToggleChatbox }) => {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      setRawResponse(raw);
      setSelectedIds((prev) =>
        prev.filter((id) => dataRows.some((row) => row.id === id)),
      );
      setActiveRow((prev) => {
  ...
  useEffect(() => {
    ...
    const normalizeMailStatus = (value) => {
      ...
      if (
        ['progress', 'in progress', 'in-progress', 'processing'].includes(txt)
      )
        return 'progress';
      return 'progress';
    };
  ...
  const applyUpdate = async (rowId, overrideForm) => {
    ...
    if (!enrollment.trim()) {
      setFlashMessage(
        'error',
        'Enrollment number is required and cannot be empty.',
      );
      return;
    }
    ...
    if (!instituteName.trim()) {
      setFlashMessage(
        'error',
        'Institute name is required and cannot be empty.',
      );
      return;
    }
    ...
    if ((nextRemark || '') !== (row.transcript_remark || ''))
      payload.transcript_remark = nextRemark || '';
    ...
    if ((nextSubmitMail || '') !== (row.submit_mail || ''))
      payload.submit_mail = nextSubmitMail || '';

    const nextPdf = form.pdf_generate ?? row.pdf_generate ?? '';
    if ((nextPdf || '') !== (row.pdf_generate || ''))
      payload.pdf_generate = nextPdf || '';

    const nextInstitute = form.institute_name ?? row.institute_name ?? '';
    if ((nextInstitute || '') !== (row.institute_name || ''))
      payload.institute_name = nextInstitute || '';

    const nextRef = form.request_ref_no ?? row.request_ref_no ?? '';
    if ((nextRef || '') !== (row.request_ref_no || ''))
      payload.request_ref_no = nextRef || '';
    ...
    try {
      ...
      setFlashMessage(
        'error',
        err.message || 'Failed to update transcript request.',
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  const handleDelete = async (id) => {
    ...
    if (!confirm('Delete this transcript request? This cannot be undone.'))
      return;
    try {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!selectedIds.length)
      return setFlashMessage('info', 'Select at least one request to delete.');
    if (
      !confirm(
        `Delete ${selectedIds.length} selected requests? This cannot be undone.`,
      )
    )
      return;
    try {
  ...
  const statusCounts = useMemo(() => {
    ...
    rows.forEach((row) => {
      ...
      else if (['progress', 'in progress', 'processing'].includes(raw))
        key = 'progress';
  ...
  const formatStatus = (value) => {
    ...
    if (['progress', 'in progress', 'in-progress', 'processing'].includes(text))
      return STATUS_LABELS.progress;
    if (['pending', 'pending approval'].includes(text))
  ...
  const statusBadgeClass = (value) => {
    ...
    if (label === STATUS_LABELS.done)
      return `${base} bg-green-100 text-green-700`;
    if (label === STATUS_LABELS.progress)
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 h-full">
      <PageTopbar
        ...
        leftSlot={
          <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-purple-600 text-white text-xl">
            📜
          </div>
        }
        actions={ACTIONS}
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (result && result.summary) {
                      ...
                      setFlashMessage(
                        'info',
                        `Imported: ${summary.created} created, ${summary.updated} updated, total ${summary.total}`,
                      );
                      // build concise modal content with sample TR numbers
                      const take = (arr, n = 20) =>
                        Array.isArray(arr) ? arr.slice(0, n) : [];
                      const createdSample =
                        take(summary.created_trs).join(', ') || '—';
                      const updatedSample =
                        take(summary.updated_trs).join(', ') || '—';
                      const modal = `Imported: ${
                        summary.created
                      } created\nUpdated: ${
                        summary.updated
                    ...
                    setFlashMessage(
                      'error',
                      err.message || 'Refresh and sync failed.',
                    );
                    setSyncModalContent(
                      err.message || 'Refresh and sync failed.',
                    );
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            {panelOpen && selectedAction === ACTIONS[1] && (
              <div className="p-4 space-y-3 text-sm text-gray-700">
                <p>
                  Apply a status to every selected request. Pick the status
                  below and click Update after choosing the rows in the table.
                </p>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      !rights.can_edit || !selectedIds.length || loading
                    }
                  >
                    Update Selected
                  </button>
                  <button
                    ...
                    disabled={
                      !rights.can_delete || !selectedIds.length || loading
                    }
                  >
                    Delete Selected
                  </button>
                  ...
                  <span className="text-xs text-gray-500">
                    Currently selected: {selectedIds.length} request(s).
                  </span>
                </div>
                <ul className="list-disc pl-5 text-xs text-gray-500 space-y-1">
                  <li>
                    Select rows using the checkboxes in the records table.
                  </li>
            ...
            {panelOpen && selectedAction === ACTIONS[2] && (
              <div
                ref={activeRowRef}
                className="p-4 space-y-4 text-sm text-gray-700"
              >
                {activeRow ? (
                  <>
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Enrollment: {activeRow.enrollment_no || 'N/A'} •
                          Reference: {activeRow.request_ref_no || 'N/A'}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span
                          className={statusBadgeClass(activeRow.mail_status)}
                        >
                    ...
                    <div className="grid grid-cols-12 gap-4 items-start">
                      ...
                      <div className="col-span-12 md:col-span-2">
                        ...
                        <div className="border border-gray-200 rounded-lg px-3 py-2 bg-gray-50 text-sm text-center font-mono">
                          {(() => {
                            const t = String(
                              activeRow.tr_request_no ??
                                activeRow.request_ref_no ??
                                'N/A',
                            );
                            if (t === 'N/A') return t;
                    ...
                    <div className="mt-3">
                      <div className="grid grid-cols-12 gap-3 items-end">
                        ...
                        <div className="col-span-12 md:col-span-1 flex md:justify-end">
                          <button
                            ...
                            disabled={
                              !rights.can_edit || updatingId === activeRow.id
                            }
                          >
                      ...
                      <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
                        <span>
                          Last updated:{' '}
                          {formatDateTime(
                            activeRow.updated_at || activeRow.modified_at,
                          )}
                        </span>
                        {rights.can_delete && (
                          <button
                  ...
                  <div className="text-sm text-gray-600">
                    Select a request from the table to review and edit details.
                  </div>
                )}
              </div>
            )}
                    ...
                    sortedRows.map((row) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 align-top">
                            {row.tr_request_no ?? row.request_ref_no ?? 'N/A'}
                          </td>
                          <td className="px-3 py-2 align-top">
                          ...
                          <td className="px-3 py-2 align-top">
                            {row.request_ref_no || 'N/A'}
                          </td>
                          <td className="px-3 py-2 align-top">
                            {row.institute_name || 'N/A'}
                          </td>
                          <td className="px-3 py-2 align-top">
                            {row.transcript_receipt || ''}
                          </td>
                          ...
                          <td className="px-3 py-2 align-top">
                            <span className={statusBadgeClass(row.mail_status)}>
                              {formatStatus(row.mail_status)}
                            </span>
                          </td>
                          <td className="px-3 py-2 align-top">
                            {row.submit_mail || 'N/A'}
          ...
          {syncModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="relative bg-white rounded-lg shadow-lg w-11/12 max-w-3xl p-4">
                ...
                <pre className="whitespace-pre-wrap max-h-96 overflow-auto text-xs bg-gray-50 p-3 rounded">
                  {syncModalContent}
                </pre>
              </div>
            </div>
