...
const Degree = ({ onToggleSidebar, onToggleChatbox }) => {
  ...
  const fetchDegrees = async () => {
    ...
    try {
      ...
      if (appliedConvocation) {
        ...
        params.convocation_no = Number.isNaN(convParam)
          ? appliedConvocation
          : convParam;
      } else if (appliedConvocationMonth) {
        const match = convocations.find(
          (conv) => conv.month_year === appliedConvocationMonth,
        );
        if (match) {
  ...
  const handleConvocationSubmit = async (e) => {
    ...
    const convNumber =
      convInput === '' || convInput === null || typeof convInput === 'undefined'
        ? null
        : Number(convInput);
    const payload = {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this degree record?'))
      return;
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const poller = setInterval(async () => {
        try {
          ...
          if (p.status === 'finished' || p.status === 'error') {
            ...
            toast.success(
              `Bulk upload finished. Created: ${p.created}, Updated: ${p.updated}`,
            );
            if (p.errors && p.errors.length > 0) {
              toast.warning(
                `${p.errors.length} rows had errors. Check details below.`,
              );
            }
            fetchDegrees();
            setUploadId(null);
      ...
      toast.error(
        'Bulk upload failed: ' + (err.response?.data?.error || err.message),
      );
      setUploadProgress(0);
    }
  };
  ...
  const handleDownloadLog = async () => {
    ...
    const id =
      uploadId ||
      (uploadResult &&
        uploadResult.log_file &&
        (() => {
          // try to parse UUID from server path like ...bulk_upload_degrees_<uuid>.log
          const m = String(uploadResult.log_file).match(
            /bulk_upload_degrees_([0-9a-fA-F-]{36})/,
          );
          return m ? m[1] : null;
        })());
    ...
    try {
      ...
      toast.error(
        'Failed to download log: ' + (err.response?.data?.error || err.message),
      );
    }
  };
...
DG002,2023002,Jane Smith,456 Park Ave Delhi,+91 9876543211,XYZ College,Master of Arts,English Literature,102,June,2023,First Class with Distinction,English,REC002,Female,1`;
  ...
  const convocationMonthYearOptions = useMemo(() => {
    ...
    convocations.forEach((conv) => {
      if (conv.month_year && !seen.has(conv.month_year)) {
        ...
        options.push({
          label: conv.month_year,
          value: conv.month_year,
          convocation_no: conv.convocation_no,
        });
      }
    });
    return options;
  ...
  const handleConvocationMonthFilterChange = (value) => {
    ...
    const match = convocationMonthYearOptions.find(
      (opt) => opt.value === value,
    );
    if (match) {
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        ...
        {panelOpen && (
          <div className="p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    >
                      ...
                      {convocations.map((conv) => (
                        <option key={conv.id} value={conv.convocation_no}>
                          Convocation {conv.convocation_no} -{' '}
                          {conv.convocation_title}
                        </option>
                      ))}
                    </select>
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleConvocationMonthFilterChange(e.target.value)
                      }
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-200 bg-white"
                    >
            ...
            {selectedMenu === '📄 Report' && <DegreeReport />}
              ...
              <tbody>
                {loading ? (
                  ...
                  degrees.map((degree, idx) => (
                    <tr
                      key={degree.id}
                      className={`border-b border-indigo-100 text-gray-700 ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.student_name_dg}
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.institute_name_dg}
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.last_exam_month}
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                </div>
            ...
            <div className="px-6 py-4">
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600 mt-2">
                  Download the template to see the required format with all
                  fields including the new Contact field.
                </p>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                <p className="text-sm text-yellow-800">
                  <strong>Note:</strong> CSV must include headers. Required
                  field: enrollment_no. The Contact field (dg_contact) is now
