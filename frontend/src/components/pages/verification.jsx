...
const Badge = ({ text }) => {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded-full bg-${color}-100 text-${color}-800`}
    >
      {text || '-'}
    </span>
  );
...
const MailBadge = ({ text }) => {
  const color =
    text === 'SENT' ? 'emerald' : text === 'FAILED' ? 'rose' : 'slate';
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded-full bg-${color}-100 text-${color}-800`}
    >
...
function Verification({
  ...
  const getSelected = () =>
    typeof selectedTopbarMenu !== 'undefined'
      ? selectedTopbarMenu
      : localSelected;
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      if (qtrim && /^(vr_|iv_|pr_|mg_|gt_)/i.test(qtrim)) {
        url = `/api/verification/?doc_rec=${encodeURIComponent(
          qtrim,
        )}&limit=50`;
      } else if (q) {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          txt = res.statusText || String(res.status);
        }
        console.error('Verification load error', res.status, txt);
        setErrorMsg(
          `Failed to load records: ${res.status} ${res.statusText}` +
            (txt ? ` - ${txt}` : ''),
        );
        setRecords([]);
        return;
      ...
      const mapped = rows.map((r) => ({
        ...
        date: isoToDMY(
          r.doc_rec_date ||
            r.date ||
            (r.doc_rec && r.doc_rec.doc_rec_date) ||
            r.createdat ||
            '',
        ),
        enrollment_no:
          r.enrollment_no || (r.enrollment && r.enrollment.enrollment_no) || '',
        enrollment: r.enrollment || null,
        second_enrollment_no:
          r.second_enrollment_no ||
          (r.second_enrollment && r.second_enrollment.enrollment_no) ||
          '',
        ...
        doc_rec_key:
          r.doc_rec_key ||
          (r.doc_rec && r.doc_rec.doc_rec_id) ||
          r.sequence ||
          r.doc_rec_id ||
          '',
        doc_rec_id:
          r.doc_rec_id ||
          (r.doc_rec && (r.doc_rec.doc_rec_id || r.doc_rec.id)) ||
          '',
  ...
  const createRecord = async () => {
    ...
    const resolveDocRecPk = async (key) => {
      ...
      try {
        const res = await fetch(
          `/api/docrec/?doc_rec_id=${encodeURIComponent(key)}`,
          { headers: { ...authHeaders() } },
        );
    ...
    if (form.doc_rec_id && String(form.doc_rec_id).trim() !== '') {
      ...
      if (
        !Number.isNaN(Number(form.doc_rec_id)) &&
        String(form.doc_rec_id).trim() !== ''
      )
        docRecPk = Number(form.doc_rec_id);
    }
    if (
      !docRecPk &&
      form.doc_rec_key &&
      String(form.doc_rec_key).trim() !== ''
    ) {
      docRecPk = await resolveDocRecPk(form.doc_rec_key);
    }

    const body = {
      doc_rec_date: (function (s) {
        if (!s) return null;
      ...
      eca_send_date: form.eca_required
        ? (function (s) {
            if (!s) return null;
            if (/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(s)) return s;
            const x = dmyToISO(s);
      ...
      eca_resubmit_date: form.eca_required
        ? form.eca_resubmit_date || null
        : null,
    ...
    <div className="p-4 md:p-6 space-y-4 h-full bg-slate-100">
      ...
      <PageTopbar
        ...
        rightSlot={
          <a
            href="/"
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800 text-white ml-2"
          >
            🏠 Home
          </a>
        }
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 border-b">
          <div className="font-semibold">
            {getSelected() ? `${getSelected()} Panel` : 'Action Panel'}
          </div>
          <button
          ...
          <div className="p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange('vr_done_date', e.target.value)
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange('enrollment_id', e.target.value)
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange('second_enrollment_id', e.target.value)
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleChange('mail_status', e.target.value)
                    }
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleChange('eca_required', e.target.value === 'Yes')
                    }
                  >
                    <option>No</option>
                    <option>Yes</option>
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        value={form.eca_name}
                        onChange={(e) =>
                          handleChange('eca_name', e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleChange('eca_ref_no', e.target.value)
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleChange('eca_send_date', e.target.value)
                        }
                      />
                    </div>
                    <div className="md:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleChange('eca_remark', e.target.value)
                        }
                      />
                    </div>
                  </>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        if (!hasDocRec) {
                          alert(
                            "Please provide Doc Rec ID before saving (field 'Doc Rec ID' or Doc Rec Key cannot be empty).",
                          );
                          return;
                        }
            ...
            {getSelected() === '📄 Report' && (
              <div className="text-sm text-gray-600">
                <p>
                  Report filters go here (date range, status, ECA yes/no,
                  counts…).
                </p>
              </div>
            )}
                ...
                {records.map((r, idx) => (
                  <tr
                    ...
                    onClick={() => {
                      ...
                      setForm({
                        ...
                        vr_done_date:
                          dmyToISO(r.vr_done_date) || r.vr_done_date || '',
                        enrollment_id:
                          r.enrollment?.id ||
                          r.enrollment_id ||
                          r.enrollment_no ||
                          '',
                        second_enrollment_id:
                          r.second_enrollment?.id ||
                          r.second_enrollment_id ||
                          r.second_enrollment_no ||
                          '',
                        name: r.student_name || '',
                        tr: r.tr_count || 0,
                        ...
                        eca_resubmit_date:
                          r.eca?.eca_resubmit_date || r.eca_resubmit_date || '',
                        ...
                        doc_rec_id:
                          ...
                          r.doc_rec_key ||
                          (r.doc_rec &&
                            (r.doc_rec.doc_rec_id || r.doc_rec.id)) ||
                          '',
                        doc_rec_key:
                          r.doc_rec_key || r.doc_rec?.doc_rec_id || '',
                      });
                      setSelected('✏️ Edit');
                      setPanelOpen(true);
                    }}
                  >
                    ...
                    <td className="py-2 px-3">
                      {r.enrollment_no || r.enrollment?.enrollment_no || '-'}
                    </td>
                    <td className="py-2 px-3">
                      {r.second_enrollment_no ||
                    ...
                    <td className="py-2 px-3">
                      {r.eca?.eca_name || r.eca_name || '-'}
                    </td>
                    <td className="py-2 px-3">
                      {r.eca?.eca_ref_no || r.eca_ref_no || '-'}
                    </td>
                    <td className="py-2 px-3">
                      {r.eca?.eca_send_date || r.eca_submit_date || '-'}
                    </td>
...
Verification.propTypes = {
  selectedTopbarMenu: PropTypes.any,
  setSelectedTopbarMenu: PropTypes.func,
