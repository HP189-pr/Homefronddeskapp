...
import { isoToDMY, dmyToISO } from '../../utils/date.js';
...
const formatMainRecord = (record) => {
  ...
  const docRecId =
    typeof record.doc_rec === 'string'
      ? record.doc_rec
      : record.doc_rec?.doc_rec_id || '';
  return {
    id: record.id ?? null,
    doc_rec: docRecId || '',
    doc_rec_date: record.doc_rec_date ? isoToDMY(record.doc_rec_date) : '',
    inst_veri_number: record.inst_veri_number || '',
    inst_veri_date: record.inst_veri_date
      ? isoToDMY(record.inst_veri_date)
      : '',
...
const NoAccessState = () => (
  <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-600">
    <p className="text-base font-semibold">
      You do not have permission to view Institutional Verification.
    </p>
...
const InstitutionalVerification = ({
  ...
  const loadList = useCallback(
    async (queryOverride = '') => {
      ...
      try {
        ...
        if (trimmed) {
          if (/^\d+$/.test(trimmed)) {
            params.set('iv_record_no', trimmed);
          } else {
            params.set('search', trimmed);
          }
        }
        const data = await fetchJSON(
          `/inst-verification-main/?${params.toString()}`,
        );
        setList(unwrapResults(data));
      } catch (err) {
        console.error(err);
        setList([]);
  ...
  const loadStudents = useCallback(
    async (docRecId) => {
      ...
      try {
        const data = await fetchJSON(
          `/inst-verification-student/?doc_rec=${encodeURIComponent(docRecId)}`,
        );
        setSRows(unwrapResults(data));
      } catch (err) {
        console.error(err);
        setSRows([]);
  ...
  useEffect(() => {
    return () => {
      ...
      if (suggestionHideTimer.current)
        clearTimeout(suggestionHideTimer.current);
  ...
  useEffect(() => {
    ...
    docRecDebounce.current = setTimeout(async () => {
      try {
        const params = new URLSearchParams();
        params.set('number', raw);
        const data = await fetchJSON(
          `/inst-verification/suggest-doc-rec/?${params.toString()}`,
        );
    ...
    recInstDebounce.current = setTimeout(async () => {
      try {
        const res = await fetchJSON(
          `/inst-verification-main/search-rec-inst/?q=${encodeURIComponent(
            value.trim(),
          )}`,
        );
  ...
  const handleRecordSelect = useCallback(
    async (record) => {
      ...
      try {
        const detail = await fetchJSON(`/inst-verification-main/${record.id}/`);
    ...
    [fetchJSON, loadStudents, resetStudentForm, setStatus],
  );
  ...
  const handleSaveMain = async (event) => {
    ...
    try {
      let data;
      if (mform.id) {
        data = await fetchJSON(`/inst-verification-main/${mform.id}/`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
      } else {
        data = await fetchJSON(`/inst-verification-main/`, {
          method: 'POST',
          body: JSON.stringify(payload),
        });
      }
  ...
  const handleStudentSave = async () => {
    ...
    try {
      if (editingStudentId) {
        await fetchJSON(`/inst-verification-student/${editingStudentId}/`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
        setStatus('Student updated successfully.');
      } else {
        await fetchJSON(`/inst-verification-student/`, {
          method: 'POST',
          body: JSON.stringify(payload),
        });
  ...
  const deleteStudent = async (row) => {
    ...
    try {
      await fetchJSON(`/inst-verification-student/${row.id}/`, {
        method: 'DELETE',
      });
  ...
  const handleAdvancedSearch = async (event) => {
    ...
    if (field === 'iv_record_no') {
      params.set('iv_record_no', term.replace(/\D/g, ''));
    } else if (field === 'inst_veri_number') {
      params.set('inst_veri_number', term);
    } else if (field === 'doc_rec') {
      params.set('doc_rec', term);
    } else {
    ...
    try {
      ...
      const records = unwrapResults(data);
    ...
    <>
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        <form className="space-y-4" onSubmit={handleAdvancedSearch}>
          ...
          <p className="text-xs text-slate-500">
            Enrollment and student-level searches currently use the best-effort
            search vector on main records.
          </p>
          {searchError && (
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <RecordsTable
          ...
          loading={searching}
          emptyLabel={
            hasSearchRun ? 'No matches.' : 'Run a search to see results.'
          }
        ...
        >
          ...
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setMForm((prev) => ({
                  ...prev,
                  inst_veri_number: value,
                  iv_record_no: autoIv || prev.iv_record_no,
                }));
              }}
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  inst_veri_date: isoToDMY(e.target.value),
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  iv_record_no: e.target.value.replace(/\D/g, ''),
                }))
              }
            />
              ...
              <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-600">
                {docRecCandidates.map((cand) => (
                  <button
                    ...
                    key={cand}
                    onClick={() =>
                      setMForm((prev) => ({ ...prev, doc_rec: cand }))
                    }
          ...
          <div className="md:col-span-3">
            ...
            <select
              ...
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, iv_status: e.target.value }))
              }
            >
          ...
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  rec_inst_sfx_name: e.target.value,
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  study_mode: e.target.value.slice(0, 1),
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, inst_ref_no: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  ref_date: isoToDMY(e.target.value),
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, doc_types: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              value={mform.rec_by}
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, rec_by: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-6">
          ...
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_address_1}
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  rec_inst_address_1: e.target.value,
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_address_2}
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  rec_inst_address_2: e.target.value,
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_city}
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, rec_inst_city: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_location}
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  rec_inst_location: e.target.value,
                }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_pin}
              onChange={(e) =>
                setMForm((prev) => ({ ...prev, rec_inst_pin: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-3">
            ...
            <input
              className="input"
              value={mform.rec_inst_email}
              onChange={(e) =>
                setMForm((prev) => ({
                  ...prev,
                  rec_inst_email: e.target.value,
                }))
              }
            />
          </div>
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm space-y-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={
                savingStudent || (!rights.can_create && !rights.can_edit)
              }
        ...
        <div className="grid gap-4 md:grid-cols-12">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              onChange={(e) =>
                setSForm((prev) => ({
                  ...prev,
                  type_of_credential: e.target.value,
                }))
              }
            />
          </div>
          <div className="md:col-span-2">
            ...
            <input
              className="input"
              value={sform.month_year}
              onChange={(e) =>
                setSForm((prev) => ({ ...prev, month_year: e.target.value }))
              }
            />
          </div>
          <div className="md:col-span-2">
            ...
            <input
              className="input"
              value={sform.verification_status}
              onChange={(e) =>
                setSForm((prev) => ({
                  ...prev,
                ...
                {srows.map((row) => (
                  <tr key={row.id || row.sr_no} className="border-t">
                    ...
                    <td className="px-2 py-1">
                      {row.enrollment || row.enrollment_no_text || '-'}
                    </td>
                    <td className="px-2 py-1">{row.student_name || '-'}</td>
                    <td className="px-2 py-1">
                      {row.type_of_credential || '-'}
                    </td>
    ...
    <div className="overflow-auto rounded-2xl border">
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {records.map((row) => {
            const docRec =
              typeof row.doc_rec === 'string'
                ? row.doc_rec
                : row.doc_rec?.doc_rec_id || '';
            return (
              <tr
                key={row.id || docRec}
                className={`border-t ${
                  clickable ? 'cursor-pointer hover:bg-slate-50' : ''
                } ${activeId && activeId === row.id ? 'bg-indigo-50' : ''}`}
