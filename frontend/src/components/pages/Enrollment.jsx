...
export default function Enrollments() {
  ...
  const getInstName = (i) =>
    i?.institute_name ?? i?.institutename ?? i?.name ?? '';
  const getInstCampus = (i) =>
    i?.institute_campus ?? i?.institutecampus ?? i?.campus ?? '';
  ...
  const getSubName = (s) =>
    s?.subcourse_name ?? s?.subcoursename ?? s?.name ?? '';
    ...
    () => [
      ...
      { field: 'student_name', headerName: 'Student Name', width: 220 },
      {
        ...
        valueGetter: (params) => {
          ...
          return i
            ? `${getInstName(i)}${
                getInstCampus(i) ? ` (${getInstCampus(i)})` : ''
              }`
            : '';
        },
      },
  ...
  async function fetchList() {
    ...
    try {
      if (mode === 'single' && q) {
        ...
        const res = await api(
          `/api/enrollments?q=${encodeURIComponent(q)}&limit=50`,
        );
        setRows(res?.rows || []);
        ...
        if (
          Array.isArray(filters.maincourse_id) &&
          filters.maincourse_id.length
        )
  ...
  function onEdit(row) {
    setEditingId(row.id);
    const ad = row.admission_date
      ? typeof row.admission_date === 'string'
  ...
  async function onSave() {
    ...
    if (
      ...
      return alert(
        'Please fill enrollment_no, student_name, institute and main course.',
      );
    }
    try {
      if (editingId) {
        await api(`/api/enrollments/${editingId}`, {
          method: 'PUT',
    ...
    return (
      <div style={{ padding: 20 }}>
        <h2 style={{ color: 'crimson' }}>
          Access Denied — you do not have permission to view enrollments.
        </h2>
      </div>
    );
  }
            ...
            <>
              <TextField
                ...
                SelectProps={{
                  ...
                  renderValue: (sel) =>
                    sel.length
                      ? sel
                          .map((v) => instById.get(v)?.institute_name || v)
                          .join(', ')
                      : 'All',
                }}
                onChange={(e) => {
                  ...
                  setFilters((f) => ({
                    ...f,
                    institute_id: Array.isArray(val) ? val : [],
                  }));
                }}
                style={{ minWidth: 260 }}
              >
                {institutes.map((i) => (
                  <MenuItem key={i.institute_id} value={i.institute_id}>
                    <Checkbox
                      checked={
                        filters.institute_id.indexOf(i.institute_id) > -1
                      }
                    />
                    <ListItemText
                      primary={`${i.institute_name}${
                        i.institute_campus ? ` (${i.institute_campus})` : ''
                      }`}
                    />
                  </MenuItem>
                ))}
              ...
              <TextField
                  ...
                  renderValue: (sel) =>
                    sel.length
                      ? sel
                          .map(
                            (v) =>
                              mainById.get(v)?.course_code ||
                              mainById.get(v)?.course_name ||
                              v,
                          )
                          .join(', ')
                      : 'All',
                ...
                onChange={(e) => {
                  ...
                  setFilters((f) => ({
                    ...f,
                    maincourse_id: Array.isArray(val) ? val : [],
                  }));
                }}
                style={{ minWidth: 240 }}
              >
                {mainCourses.map((m) => (
                  <MenuItem key={m.maincourse_id} value={m.maincourse_id}>
                    <Checkbox
                      checked={
                        filters.maincourse_id.indexOf(m.maincourse_id) > -1
                      }
                    />
                    <ListItemText
                      primary={`${m.course_code || ''} ${
                        m.course_name || ''
                      }`.trim()}
                    />
                  </MenuItem>
              ...
              <TextField
                ...
                size="small"
                label="Sub Course"
                value={filters.subcourse_id}
                SelectProps={{
                  multiple: true,
                  renderValue: (sel) =>
                    sel.length
                      ? sel
                          .map((v) => subById.get(v)?.subcourse_name || v)
                          .join(', ')
                      : 'All',
                }}
                onChange={(e) => {
                  ...
                  setFilters((f) => ({
                    ...f,
                    subcourse_id: Array.isArray(val) ? val : [],
                  }));
                }}
                style={{ minWidth: 240 }}
              >
                {subCourses
                  .filter(
                    (s) =>
                      !(
                        filters.maincourse_id && filters.maincourse_id.length
                      ) || filters.maincourse_id.includes(s.maincourse_id),
                  )
                  .map((s) => (
                    <MenuItem key={s.subcourse_id} value={s.subcourse_id}>
                      <Checkbox
                        checked={
                          filters.subcourse_id.indexOf(s.subcourse_id) > -1
                        }
                      />
              ...
              <TextField
                size="small"
                label="Batch (comma-separated)"
                placeholder="e.g. 2020,2021"
                value={filters.batch}
                onChange={(e) =>
                  setFilters((f) => ({ ...f, batch: e.target.value }))
                }
              />
            </>
          )}
      ...
      >
        ...
        <DialogContent dividers>
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <TextField
              label="Temp Enroll No"
              value={form.temp_enroll_no}
              onChange={(e) =>
                setForm({ ...form, temp_enroll_no: e.target.value })
              }
            />
            <TextField
              label="Student Name"
            ...
            <TextField
              select
              label="Main Course"
              value={form.maincourse_id}
              onChange={(e) =>
                setForm({ ...form, maincourse_id: e.target.value })
              }
            >
            ...
            <TextField
              ...
              onChange={(e) =>
                setForm({ ...form, subcourse_id: e.target.value })
              }
            >
              ...
              {subCourses
                .filter(
                  (s) =>
                    !form.maincourse_id ||
                    s.maincourse_id === form.maincourse_id,
                )
                .map((s) => (
                  <MenuItem key={s.subcourse_id} value={s.subcourse_id}>
                    {s.subcourse_name}
