...
const Enrollment = ({
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/my-navigation/`, {
                ...
                {state.filteredEnrollments.map((enr) => (
                  <tr key={enr.enrollment_no}>
                    ...
                    <td className="border p-2">
                      {enr.institute?.name || enr.institute_id}
                    </td>
                    <td className="border p-2">
                      {enr.maincourse?.name || enr.maincourse_id}
                    </td>
                    <td className="border p-2">
                      {enr.subcourse?.name || enr.subcourse_id}
                    </td>
                    <td className="border p-2">{enr.batch}</td>
                    <td className="border p-2">
                      {isoToDMY(enr.admission_date) || '-'}
                    </td>
                    {(rights.can_edit || rights.can_delete) && (
                      <td className="border p-2">
                        {rights.can_edit && (
    ...
    <div>
      ...
      {!rights.can_create && !formState.isEditing && (
        <p className="text-sm text-red-600 mb-2">
          You do not have rights to create enrollments.
        </p>
      )}
      {!rights.can_edit && formState.isEditing && (
        <p className="text-sm text-red-600 mb-2">
          You do not have rights to edit enrollments.
        </p>
      )}
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
        ...
        {panelOpen && getSelected() && (
          <div className="p-4">
            ...
            {getSelected() === '🔍' && (
              <div className="space-y-4">{renderSearchView()}</div>
            )}
