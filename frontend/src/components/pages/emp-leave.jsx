...
function getFiscalYear(dateInput) {
  const dt =
    dateInput instanceof Date
      ? dateInput
      : parseDMY(dateInput) || new Date(dateInput);
...
const EmpLeavePage = () => {
  ...
  const baseFieldClass =
    'w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500';
    ...
    return (leaveEntries || [])
      .filter((le) => {
        ...
        if (recordSearch.trim()) {
          ...
          const reportHit = String(le.leave_report_no || '')
            .toLowerCase()
            .includes(q);
          const empIdHit = String(le.emp || '')
            .toLowerCase()
            .includes(q);
          const empNameHit = String(le.emp_name || '')
            .toLowerCase()
            .includes(q);
          ...
          <div>
              ...
              <div className="p-4">
                ...
                <form onSubmit={handleApply} className="mt-4 space-y-4">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const p = profiles.find(
                            (pp) =>
                              String(pp.emp_id) === String(e.target.value),
                          );
                          if (p)
                            setForm((f) => ({ ...f, emp_name: p.emp_name }));
                        }}
                        className={baseFieldClass}
                      >
                  ...
                  <div className="flex flex-col gap-3 md:flex-row">
                    ...
                    <div className="flex flex-col gap-1 md:w-40">
                      ...
                      >
                        ...
                        {leaveTypes.map((lt) => (
                          <option
                            key={lt.leave_code || lt.id}
                            value={lt.leave_code || lt.id}
                          >
                    ...
                    <div className="flex flex-col gap-1 md:w-24">
                      ...
                      <select
                        ...
                        value={form.status}
                        onChange={handleChange}
                        className={`${baseFieldClass} border-gray-900 text-gray-900`}
                      >
                  ...
                  <div className="flex gap-3 pt-2 border-t border-gray-100">
                    <button
                      type="submit"
                      className="px-5 py-2 bg-indigo-600 text-white rounded shadow-sm hover:bg-indigo-500"
                    >
                      {editingId ? 'Save' : 'Add'}
                    </button>
                    <button
            ...
            <div className="border rounded-xl overflow-hidden mt-4">
                    ...
                    {filteredEntries.length === 0 ? (
                      ...
                      filteredEntries.map((le) => (
                        <tr
                          ...
                          onClick={() => {
                            const p = profiles.find(
                              (px) => String(px.emp_id) === String(le.emp),
                            );
                            setForm({
                        ...
                        >
                          ...
                          <td className="p-2">
                            {le.leave_type_name || le.leave_type}
                          </td>
                          <td className="p-2">
                            {fmtDate(le.start_date)} - {fmtDate(le.end_date)}
                          </td>
              ...
              <div className="text-xs p-2 bg-gray-50 text-gray-500">
                Click a row to open it in the Add panel.
              </div>
            </div>
          </div>
        )}
